...
"syntax region gbkFeature     start=/^     [a-zA-Z]\+/ end=/^     [a-zA-Z]\+/re=e-9 contains=gbkFeatureAnnotation keepend

"syntax region gbkFeatureAnnotation start=/^                     \/[a-zA-Z_]*/ end=/^                     \/[a-zA-Z_]*/ end=/^     [a-zA-Z]\+/ contained

syntax match gbkFeatureType /^     [a-zA-Z][a-zA-Z]*/ containedin=gbkFeature
syntax match gbkFeatureLocation /\(complement(\)\?<\?\d\+\.\.>\?\d\+)\?/ containedin=gbkFeatureLine
